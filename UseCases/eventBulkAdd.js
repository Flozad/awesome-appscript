...
function getEvents() {
  ...
  var cal = CalendarApp.getCalendarById(
    'id_del_calendario_que_va_a_usarse@group.calendar.google.com'
  );
  var events = cal.getEvents(
    new Date('27/12/2017 02:16 AM'),
    new Date('27/12/2017 02:30 PM')
  );
...
function addEvents() {
  ...
  for (var i = 0; i < data.length; i++) {
    cal.createEvent(data[i][0], data[i][1], data[i][2], {
      location: data[i][3],
